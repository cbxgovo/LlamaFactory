...
def _is_local_xing4_0_model(model_path: str) -> bool:
    ...
    try:
        with open(config_path, encoding="utf-8") as f:
            return json.load(f).get("model_type") == "xing4_0"
    except Exception:
        return False
...
def _get_init_kwargs(model_args: "ModelArguments") -> dict[str, Any]:
    ...
    if not model_args.trust_remote_code and _is_local_xing4_0_model(model_args.model_name_or_path):
        model_args.trust_remote_code = True
        logger.warning_rank0("Xing4.0 uses local custom auto_map code, set `trust_remote_code` to True.")
